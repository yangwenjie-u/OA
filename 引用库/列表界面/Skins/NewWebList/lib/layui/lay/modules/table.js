...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function(exports) {
    ...
    var $ = layui.$,
        ...
        thisTable = function() {
            ...
            return {
                ...
                //外部可以调用的函数
                //发起ajax请求
                pullData: function(options, handle) {
                    that.pullData(1, '', options, handle);
                },
                //排序
                sort: function() {
    ...
    //获得数据
    Class.prototype.pullData = function(curr, loadIndex, curParams, handle) { //curParams changed
        var that = this,
            options = that.config,
        ...
        if (options.url) { //Ajax请求
            ...
            $.ajax({
                ...
                success: function(res) {
                    ...
                    typeof handle === 'function' ? handle(res) : ''; //handle(res);changed
                },
                error: function(e, m) {
                    layer.close(loadWinIndex);
    ...
    Class.prototype.sort = function(th, type, pull, formEvent) {
        ...
        //changed--begin
        try {
            // type  asc升序   desc降序   null
            options.curtomSort(type, field);
            if (!type) {
                delete that.sortKey;
            }
    ...
    Class.prototype.scrollPatch = function() {
        ...
        if (scollWidth && scollHeight) {
            if (!that.elem.find('.layui-table-patch')[0]) {
                ...
                //changed--begin
                //设置table头的最后一个元素的宽度等于table内容的最后一个元素
                var tab1 = that.layHeader.eq(0).find('.layui-table')
                var tab2 = that.layMain.find('.layui-table')
                if (tab1.width() > tab2.width()) {
                    var wid2 = tab2.find('tr:first').find('td:visible:last').width();
                    tab1.find('tr:last').find('th:visible:last div').width(wid2);
                }
                //changed--end
    ...
    Class.prototype.events = function() {
        ...
        th.on('mousemove', function(e) {
            ...
            if (dict.allowResize) {
                ...
                that.getCssRule(field, function(item) {
                    ...
                    dict.rule = item;
                    // changed-begin
                    dict.rule2 = othis.parent().next().children(":visible").eq(othis.index()).children("div");
                    var fixed = othis.parents(".layui-table-fixed");
                    if (fixed.length) {
                        dict.rule3 = othis.parents('.layui-table-box').find(".layui-table-header table thead tr").eq(1).children(":visible").eq(othis.index()).children("div");
                    }
                    // changed-end
                    dict.ruleWidth = parseFloat(width);
                    // console.log("ruleWidth:" +dict.ruleWidth )
        ...
        _DOC.on('mousemove', function(e) {
            if (dict.resizeStart) {
                ...
                if (dict.rule) {
                    ...
                    // console.log(dict.rule);
                    // console.log(setWidth + 'px');
                    // changed-begin
                    if (dict.rule2 && dict.rule2.length) {
                        dict.rule2.width(setWidth + 'px');
                    }
                    if (dict.rule3 && dict.rule3.length) {
                        dict.rule3.width(setWidth + 'px');
                    }
                    // changed-end
        ...
        //                 }
        //                 return '';
        //             }() + '">', elemCell.html(), '</div>', '<i class="layui-icon layui-table-tips-c">&#x1006;</i>'
        //         ].join(''), elemCell[0], {
        //             tips: [3, ''],
        //             time: -1,
        //             anim: -1,
