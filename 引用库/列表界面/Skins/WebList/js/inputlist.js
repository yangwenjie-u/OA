...
function addRecord() {
    $.ajax({
        ...
        dataType: 'json',
        data: { param: $("#param").val(),jsonData: JSON.stringify(defRow) },
        success: function (data) {
            if (data.success) {
                dataGrid.jqxGrid('addrow', null, data.data, "first");
                //dataGrid.jqxGrid('updatebounddata');
            }
            else
            {
...
function delRecord() {
    //判断是否选择
    var selIndex = dataGrid.jqxGrid('getselectedrowindexes');
    if (selIndex == "") {
        alert("请选择记录信息");
        return;
    }
    ...
    //询问框
    if(confirm("确定删除数据吗？")){
        var data = dataGrid.jqxGrid('getrowdata', selIndex[0]);
        $.ajax({
            type: "post",
            url: '/WebList/DelData',
            dataType: 'json',
            data: { param: $("#param").val(), jsonData: JSON.stringify(data) },
            success: function (data) {
                if (data.success) {
                    //dataGrid.jqxGrid('deleterow', selIndex[0]);
                    dataGrid.jqxGrid('updatebounddata');
                    layer.alert(data.msg);
                }
                else {
