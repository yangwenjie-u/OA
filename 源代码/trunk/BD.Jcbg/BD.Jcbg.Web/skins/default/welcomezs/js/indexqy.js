...
function init() {
    ...
    var point = new BMap.Point(120.705, 28.001);
    ...
    $("#moresearch").css("top", topheightbottom);
}
...
function gc_mapstyle() {
    ...
    ShowLegendControl.prototype.initialize = function (map) {
        ...
        ul.style.opacity = " 1";
        var li = $("<li id='construcstyle'>在建工程<img src='/skins/default/welcomezs/images/zaijian.png'/></li>").appendTo(ul);
        var li = $("<li id='construcstyle'>完工工程<img src='/skins/default/welcomezs/images/wangong.png'/></li>").appendTo(ul);
        var li = $("<li id='construcstyle'>停工工程<img src='/skins/default/welcomezs/images/tigong.png'/></li>").appendTo(ul);
        // 添加DOM元素到地图中
...
function qy_mapstyle() {
    ...
    ShowLegendControl.prototype.initialize = function (map) {
        ...
        ul.style.padding = "15px";
        ul.style.opacity = " 1";
        var li = $("<li id='construcstyle'>所有单位<img src='/skins/default/welcomezs/images/qyzt.png'/></li>").appendTo(ul);
        // 添加DOM元素到地图中
        map.getContainer().appendChild(ul);
        // 将DOM元素返回
        return ul;
...
function ry_mapstyle() {
    ...
    ShowLegendControl.prototype.initialize = function (map) {
        ...
        ul.style.padding = "15px";
        ul.style.opacity = "1";
        var li = $("<li id='construcstyle'>在职<img src='/skins/default/welcomezs/images/qyzt.png'/></li>").appendTo(ul);
        var li = $("<li id='construcstyle'>离职<img src='/skins/default/welcomezs/images/wangong.png'/></li>").appendTo(ul);
        // 添加DOM元素到地图中
        map.getContainer().appendChild(ul);
        // 将DOM元素返回
        return ul;
    ...
    for (var i = 0; i < points.length; i++) {
        ...
        if (gcbz != "") {
            ...
            if (points[i].sy_gczt == "在建工程") {
                var myIcon = new BMap.Icon("/skins/default/welcomezs/images/zaijian.png", new BMap.Size(95, 95));
            } else if (points[i].sy_gczt == "竣工工程") {
                var myIcon = new BMap.Icon("/skins/default/welcomezs/images/wangong.png", new BMap.Size(95, 95));
            } else {
                var myIcon = new BMap.Icon("/skins/default/welcomezs/images/tigong.png", new BMap.Size(95, 95));
            }
...
function showInfo(thisMaker, point) {
    ...
    var enginnersingle = " ";
    enginnersingle += "<div id='enginner_bigbox'><p class='enginner_bigbox_title'>工程名称:" + point.gcmc + "</p><div id='enginner_bigbox_left'><p class='enginner_bigbox_all'>施工单位:" + point.sgdwmc + "</p><p class='enginner_bigbox_all'>考勤人数:" + point.kqrs + "</p><p class='enginner_bigbox_all'>登记人数:" + point.zcrs + "</p><p class='enginner_bigbox_all'>在职人数:" + point.zzrs + "</p><p class='enginner_bigbox_all'>离职人数:" + point.lzrs + "</p><p class='enginner_bigbox_all'>考勤机状态:" + point.zxkq + '/' + point.kqjs + "</p></div></div>";
...
function addMarker2(points) {
    for (var i = 0; i < points.length; i++) {
        if (points[i].lon != "") {
            ...
            var myIcon = new BMap.Icon("/skins/default/welcomezs/images/qyzt.png", new BMap.Size(95, 95));
            var marker = new BMap.Marker(point, {
                icon: myIcon
            });
            map.addOverlay(marker);
...
function setRylist() {
    ...
    $.each(rydata, function (index, value) {
        rysingle += "<div class='rysinglelist clearfix'><div class='singlelist_left'><p>姓名:" + value.ryxm + "</p><p>身份证号:" + value.sfzhm + "</p><p>所属工程:" + value.gcmc + "</p><p>状态:" + (value.hasdelete ? "在职" : "离职") + "</p><p style='color:blue' onclick='tolskq(&quot;" + value.sfzhm + "&quot;,&quot;" + value.sfbzfzr + "&quot;)'>历史考勤</p></div><div class='singlelist_right'><img src='data:image/png;base64," + ((value.zp == "" || typeof (value.zp) == "undefined") ? defimg : value.zp) + "'/></div></div>"
    });
...
function initGcTable() {
    ...
    layui.use('table', function () {
        ...
        table.render({
            elem: '#gcTable'
          , url: "/welcome/GetGcList?province=" + province + "&city=" + city + "&district=" + district + "&qylx=" + qylx + "&qybh=" + qybh + "&gczt=" + gczt
          ...
          , cols: [[
            { field: 'gcmc', width: 350, title: '工程名称', templet: '#gcmcTpl' },
            { field: 'gclxbh', width: 100, title: '工程类型' },
            { field: 'sy_gczt', width: 100, title: '工程状态' },
            { field: 'jsdwfzr', width: 100, title: '负责人' },
            { field: 'jsdwlxdh', width: 100, title: '联系电话' },
            { field: 'cql', width: 350, title: '出勤情况', templet: '#cqlTpl' },
            { field: 'ffl', width: 350, title: '上月工资发放情况', templet: '#fflTpl' }
          ]]

        });

        //高度调整
        var tableheight = $(".panel").height() - 190;
...
function initQyTable() {
    ...
    layui.use('table', function () {
        ...
        table.render({
            elem: '#qyTable'
          , url: "/welcome/GetQyList?province=" + province + "&city=" + city + "&district=" + district + "&qylx=" + qylx + "&qybh=" + qybh + "&gczt=" + gczt
          , page: { //支持传入 laypage 组件的所有参数（某些参数除外，如：jump/elem） - 详见文档
              layout: ['limit', 'count', 'prev', 'page', 'next', 'skip'] //自定义分页布局
              //,curr: 5 //设定初始在第 5 页
            , groups: 1 //只显示 1 个连续页码
...
function initRyTable() {
    ...
    layui.use('table', function () {
        ...
        table.render({
            elem: '#ryTable'
          , url: "/welcome/GetRyList?province=" + province + "&city=" + city + "&district=" + district + "&qylx=" + qylx + "&qybh=" + qybh + "&gczt=" + gczt
          , page: { //支持传入 laypage 组件的所有参数（某些参数除外，如：jump/elem） - 详见文档
              layout: ['limit', 'count', 'prev', 'page', 'next', 'skip'] //自定义分页布局
              //,curr: 5 //设定初始在第 5 页
            , groups: 1 //只显示 1 个连续页码
