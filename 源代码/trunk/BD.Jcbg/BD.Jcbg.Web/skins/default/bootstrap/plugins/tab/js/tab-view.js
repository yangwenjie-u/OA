...
************************************************************************************************************/
	...
	var tabView_maxNumberOfTabs = 100;	// Maximum number of tabs
	...
	function selectTab(parentId, title) {
	    try{
	        var index = getTabIndexByTitle(title);
	        alert(index);
	        if (index > -1)
	            showTab(parentId, index);
	    } catch (e) {
	...
	function showTab(parentId,tabIndex)
	{
		...
		if(activeTabIndex[parentId]>=0){
			...
			img.src =g_rootImagePath+ 'images/tab_right_inactive.gif';
			img.className="tab_right_inactive_class";
		...
		img.src = g_rootImagePath + 'images/tab_right_active.gif';
		img.className="tab_right_inactive_class";
	...
	function rolloverTab()
	{
		if(this.className.indexOf('tabInactive')>=0){
			...
			img.src = g_rootImagePath + 'images/tab_right_over.gif';
		}
	...
	var g_rootImagePath = "";
	function initTabs(rootImagePath, mainContainerID, tabTitles, activeTab, width, height, closeButtonArray, additionalTab)
	{
	    if (rootImagePath != "")
    	    g_rootImagePath = rootImagePath;
		...
		if(!additionalTab || additionalTab=='undefined'){
			...
			//var temp_zhuye= document.createElement('DIV');
		//	temp_zhuye.className="temp_zhuye_class";
			//temp_zhuye.innerHTML=’<span>这是被创建出来的div3</span>’; 
		//	tabDiv.insertBefore(temp_zhuye); //在大的div元素下插入创建出来的元素，第一个参数是创建的div，第二个参数是要插入到哪个div前面。
		...
		for(var no=0;no<tabTitles.length;no++){
			...
			if(closeButtonArray[no]){
				...
				closeButton.src = g_rootImagePath + 'images/close.gif';
				closeButton.className="close_tab_div";
		...
		for(var no=0;no<tabs.length;no++){
			if(tabs[no].className=='dhtmlgoodies_aTab' && tabs[no].parentNode.id == mainContainerID){
				if(height.length>0)tabs[no].style.height =(parseInt(height)-35)+"px";
	...
	function getTabCount(parentId) {
	    return tabView_countTabs[parentId];
	}
