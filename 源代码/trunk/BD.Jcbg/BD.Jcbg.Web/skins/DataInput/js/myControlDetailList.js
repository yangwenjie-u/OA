...
function GetT3Tab(tablab, table, sy, rec) {
    ...
    html += "<div class=\"tabs-container\" style=\"height:300px\">";
    //显示列表
    //html += "<input type='hidden' id='" + table + "CHECKDATA_" + suffix + "' name='" + table + "CHECKDATA_" + suffix + "' value='1'/>";
    // html += "<input type='hidden' id='" + table + "PRIDATA_" + suffix + "' name='" + table + "PRIDATA_" + suffix + "' value=''/>";
    //字段内容
    var fieldcontent = $("#" + table + "Div").html();
    var regField = new RegExp("&amp;&amp;record&amp;&amp;", "g");
    fieldcontent = fieldcontent.replace(regField, rec);
    var regcodeField = new RegExp("&&record&&", "g");
    fieldcontent = fieldcontent.replace(regcodeField, rec);
    //替换数据属性
    ...
    jscontent = jscontent.replace(regField, rec);
    jscontent = jscontent.replace(regcodeField, rec);
    html += jscontent;
    return html;
}
...
function CreateListJs(ptable, table, triggerfield, field, presuffix, suffix) {
    ...
    //新建默认值
    var defrow = "";
    defrow += "    var " + table + presuffix + suffix + "defRow = ";
    defrow += "    {";
    defrow += "        \"opt\":\"new\",";
    //循环
    ...
    $(field).each(function (index) {
        ...
        if (field[index].kjlx == "SELECT") {
            ...
            if (ctrlChange != "") {
                ...
                fieldStr += "                   changeOtherContrl(" + table + presuffix + suffix + "dataGrid," + table + presuffix + suffix + "editIndex,\"" + ctrlChange + "\");";
                fieldStr += "           },";
            }
            ...
            fieldStr += "   editor: {";
            fieldStr += "       type:'validatebox',"; //textbox，validatebox
            fieldStr += "       options:{";
            //fieldStr += "           required: true";
            fieldStr += "       }";
            fieldStr += "   },";
    ...
    ret += "            onClickCell: " + table + presuffix + suffix + "onClickCell,";
    if(!dataReadonly)
        ret += "            toolbar: '#" + table + presuffix + suffix + "_toolbar',";
    ret += "            frozenColumns: [[{field:'ck',checkbox:true}]]";
    ret += "        });"
    //渲染
    ret += "        $.parser.parse($('#" + table + presuffix + suffix + "_toolbar'));";

    ret += "    }";

    //获取DataGrid的JSON格式
    ret += "    function " + table + presuffix + suffix + "SetSaveData() {";
    ret += "        var rows = " + table + presuffix + suffix + "dataGrid.datagrid('getRows');";
    ret += "        var json = '';";
    ...
    ret += "                json += '{';";
    ret += "                if(rows[i]." + table + "pri == undefined) ";
    ret += "                    json += '\"" + table + "pri\":\"\",';";
    ret += "                else";
    ret += "                    json += '\"" + table + "pri\":\"' + rows[i]." + table + "pri + '\",';";    
    $(field).each(function (index) {
    ...
    ret += "            json += ']';";
    ret += "            json += '}';";
    ret += "            $('#" + table + presuffix + suffix + "SaveData').val(json);";
    ret += "    }";

    //点击
    ret += "    function " + table + presuffix + suffix + "onClickCell(index, field) {";
    ret += "        if (" + table + presuffix + suffix + "editIndex != index && " + table + presuffix + suffix + "editIndex != undefined) {";
    ret += "            $.messager.confirm('询问', '记录还处于编辑状态，是否撤销编辑信息?', function (r) {";
    ret += "                if (r) {";
    ret += "                    " + table + presuffix + suffix + "dataGrid.datagrid('rejectChanges');";
    ret += "                    " + table + presuffix + suffix + "editIndex = undefined;";
    ret += "                }";
    ret += "            });";
    ret += "        }";
    ret += "    }";
    ...
    //添加
    ret += "    function new" + table + presuffix + suffix + "() {";
    ret += "        try {";
    //判断是否还在编辑状态
    ret += "            if (" + table + presuffix + suffix + "editIndex != undefined) {";
    ret += "                $.messager.alert('提示', '记录还处于编辑状态！', 'warning');";
    ret += "                return;";
    ret += "            }";
    ret += "            var newRow = {};";
    ret += "            $.extend(newRow, " + table + presuffix + suffix + "defRow);";
    ret += "            " + table + presuffix + suffix + "dataGrid.datagrid('insertRow', {";
    ret += "                index: 0,";
    ret += "                row: newRow";
    ret += "            });";
    //设置编辑行
    ret += "            " + table + presuffix + suffix + "editIndex = 0;";
    ret += "            " + table + presuffix + suffix + "dataGrid.datagrid('selectRow').datagrid('beginEdit', " + table + presuffix + suffix + "editIndex);";
    ret += "            var editors = " + table + presuffix + suffix + "dataGrid.datagrid('getEditors', " + table + presuffix + suffix + "editIndex);";
    //定义事件
    //循环
    $(field).each(function (index) {
        if (field[index].helplink.length > 0) {
            ret += "    editors[" + index + "].target.bind('keydown',function(event) {";
            ret += "        ShowListHelpLink(" + table + presuffix + suffix + "dataGrid,editors," + table + presuffix + suffix + "editIndex,event,'" + field[index].helplink + "','" + table + presuffix + suffix  + "')";
            ret += "    });";
        }
    });
    ...
    ret += "        }";
    ret += "        catch (e) {";
    ret += "            alert('new" + table + presuffix + suffix + "出错，原因：' + e.Message);";
    ret += "        }";
    ret += "    }";

    //修改
    ret += "    function modify" + table + presuffix + suffix + "() {";
    ret += "        try {";
    //判断是否还在编辑状态
    ret += "            if (" + table + presuffix + suffix + "editIndex != undefined) {";
    ret += "                $.messager.alert('提示', '记录还处于编辑状态！', 'warning');";
    ...
    ret += "            }";
    //判断是否已经选择
    ret += "            var rows = " + table + presuffix + suffix + "dataGrid.datagrid('getSelected');";
    ret += "            if (!rows || rows.length == 0) {";
    ret += "                $.messager.alert('提示', '请先择需要操作的记录！', 'warning');";
    ret += "                return;";
    ret += "            }";
    //设置编辑行
    ret += "            " + table + presuffix + suffix + "editIndex = " + table + presuffix + suffix + "dataGrid.datagrid('getRowIndex', rows);";
    ret += "            " + table + presuffix + suffix + "dataGrid.datagrid('selectRow').datagrid('beginEdit', " + table + presuffix + suffix + "editIndex);";
    ret += "            var editors = " + table + presuffix + suffix + "dataGrid.datagrid('getEditors', " + table + presuffix + suffix + "editIndex);";
    //定义事件
    //循环
    ...
    ret += "        }";
    ret += "        catch (e) {";
    ret += "            alert('modify" + table + presuffix + suffix + "出错，原因：' + e.Message);";
    ret += "        }";
    ret += "    }";

    //取消
    ret += "    function cancel" + table + presuffix + suffix + "() {";
    ret += "        try {";
    ret += "        if (" + table + presuffix + suffix + "editIndex != undefined) {";
    ret += "            $.messager.confirm('询问', '记录还处于编辑状态，是否撤销编辑信息?', function (r) {";
    ret += "                if (r) {";
    ...
    ret += "        }";
    ret += "        catch (e) {";
    ret += "            alert('cancel" + table + presuffix + suffix + "出错，原因：' + e.Message);";
    ret += "        }";
    ret += "    }";

    //保存
    ret += "    function save" + table + presuffix + suffix + "() {";
    ret += "        try {";
    //判断是否还在编辑状态 
    ret += "            if (" + table + presuffix + suffix + "editIndex == undefined) {";
    ret += "                $.messager.alert('提示', '记录未处于编辑状态！', 'warning');";
    ret += "                return;";
    ret += "            }";

    ret += "            if (!" + table + presuffix + suffix + "dataGrid.datagrid('validateRow', " + table + presuffix + suffix + "editIndex)) {";
    ret += "                $.messager.alert('提示', '请输入必输项！', 'warning');";
    ret += "                return;";
    ret += "            }";
    //结束编辑
    ret += "            " + table + presuffix + suffix + "dataGrid.datagrid('endEdit', " + table + presuffix + suffix + "editIndex);";
    ret += "            " + table + presuffix + suffix + "dataGrid.datagrid('acceptChanges');";
    ret += "            " + table + presuffix + suffix + "editIndex = undefined;";
    ret += "            " + table + presuffix + suffix + "SetSaveData();";
    ret += "        }";
    ret += "        catch (e) {";
    ret += "            alert('save" + table + presuffix + suffix + "出错，原因：' + e.Message);";
    ret += "        }";
    ret += "    }";

    //删除
    ret += "    function del" + table + presuffix + suffix + "() {";
    ret += "        try {";
    ret += "            if (" + table + presuffix + suffix + "editIndex != undefined) {";
    ret += "                $.messager.alert('提示', '记录未处于编辑状态！', 'warning');";
    ret += "                return;";
    ret += "            }";
    ret += "            var row = " + table + presuffix + suffix + "dataGrid.datagrid('getSelected');";
    ...
    ret += "                    return;";
    ret += "                }";
    ret += "            var rowIndex = " + table + presuffix + suffix + "dataGrid.datagrid('getRowIndex', row);";
    ret += "            " + table + presuffix + suffix + "dataGrid.datagrid('deleteRow', rowIndex);";
    ret += "            " + table + presuffix + suffix + "SetSaveData();"; 
    ret += "            });";
    ret += "        }";
    ret += "        catch (e) {";
    ret += "            alert('del" + table + presuffix + suffix + "出错，原因：' + e.Message);";
    ret += "        }";
    ret += "    }";

    //复制
    ret += "    function copy" + table + presuffix + suffix + "() {";
    ret += "        try {";
    ret += "            if (" + table + presuffix + suffix + "editIndex != undefined) {";
    ret += "                $.messager.alert('提示', '记录未处于编辑状态！', 'warning');";
    ret += "                return;";
    ...
    ret += "            }";
    ret += "            var newRow = {};";
    ret += "            $.extend(newRow, rows);";
    //清除原有主键
    ret += "            newRow." + table + "pri = \"\";";
    ret += "            " + table + presuffix + suffix + "dataGrid.datagrid('insertRow', {";
    ret += "                index: 0,";
    ret += "                row: newRow";
    ret += "            });";
    ...
    ret += "        }";
    ret += "        catch (e) {";
    ret += "            alert('copy" + table + presuffix + suffix + "出错，原因：' + e.Message);";
    ret += "        }";
    ret += "    }";
    ret += "</script>";
    //base编码
...
function CreateGridListDiv(table, hiddenfield, copyhiddenfield, field, presuffix, suffix) {
    var ret = "";
    ret += "";
    ret += "<input type='hidden' id='" + table + presuffix + suffix + "SaveData' name='" + table + presuffix + suffix + "SaveData' value=''/>";
    ret += "<input type='hidden' id='" + table + presuffix + suffix + "CheckData' name='" + table + presuffix + suffix + "CheckData' value='1'/>";
    ...
    ret += "    <a href='javascript:void(0)' id='btn_save_" + table + presuffix + suffix + "' class='easyui-linkbutton' iconCls='icon-save' plain='true' onclick='save" + table + presuffix + suffix + "()'>保存</a>";
    ret += "    <a href='javascript:void(0)' id='btn_cancel_" + table + presuffix + suffix + "' class='easyui-linkbutton' iconCls='icon-cancel' plain='true' onclick='cancel" + table + presuffix + suffix + "()'>取消</a>";
...
function changeOtherContrl(dataGrid, editIndex, changeStr) {
    ...
    for (var i = 0; i < typeList.length; i++) {
        ...
        if (keyParam.key == "ctrlChange") {
            ...
            $.ajax({
                type: "POST",
                url: "/DataInput/CtrlStringDataEasyUI",
                dataType: "json",
                async: false,
