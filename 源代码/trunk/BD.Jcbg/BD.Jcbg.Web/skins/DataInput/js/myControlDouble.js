        ...
        if (field[index].sfgd) {
            ...
            switch (field[index].kjlx) {
                ...
                case "BOOTSTRAPSINGSELECT":
                    ...
                    break;
                case "BOOTSTRAPMULSELECT2":
                    fieldRowStr += CreateBootstrapMulSelect2(field[index], presuffix + suffix);
                    break;
                default:
                    fieldRowStr += CreateHtmlText(field[index], presuffix + suffix);
                    break;
            ...
            switch (field[index].kjlx) {
                ...
                case "BOOTSTRAPSINGSELECT":
                    ...
                    break;
                case "BOOTSTRAPMULSELECT2":
                    ret += CreateBootstrapMulSelect2(field[index], presuffix + suffix);
                    break;
                default:
                    ret += CreateHtmlText(field[index], presuffix + suffix);
                    break;
...
//多选下拉过滤2
function CreateBootstrapMulSelect2(ctrl, suffix) {
    var ret = "";
    ret += "<select custom='custom' datain='&&datain&&' id='" + ctrl.fieldname + suffix + "' name = '" + ctrl.fieldname + suffix + "' mc = '" + ctrl.sy + "' multiple='multiple' ";
    //控件信息
    ret += "kjlx='" + ctrl.kjlx + "' inputlx='" + ctrl.inputlx + "' ";
    ...
    if (ctrl.mustin)
        ret += "mustin='mustin' ";
    /* style='width:350px;'*/
    ret += "multiple data-live-search='true' ";
    //下拉框值
    var valueStr = "";
    var ctrlstring = "";
    var funStr = "";
...
//** 从表 **
//从表Tab项模板
function GetT2Tab(table, sy, hasDetail, detail, detailsy, detailsyfield) {
    ...
    html += " <span  class=\"btn btn-default btn-add\" id=\"" + table + "AddBtn\" name=\"" + table + "AddBtn\"  onclick=\"AddT2Page('" + table + "','" + sy + "'," + hasDetail + ",'" + detail + "','" + detailsy + "','" + detailsyfield + "');\"></span>";
    html += " <span class=\"btn btn-default btn-copy\" id=\"" + table + "CopyBtn\" name=\"" + table + "CopyBtn\"   onclick=\"CopyT2Page('" + table + "','" + sy + "'," + hasDetail + ",'" + detail + "','" + detailsy + "','" + detailsyfield + "');\"></span>";
    ...
    html += "   </div>"
    html += "   <div class='left-nav' id=\"" + table + "Nav\">";
...
//detail:明细表名
//rec:当前从表记录数
function AddT2Page(table, sy, hasDetail, detail, detailsy, detailsyfield) {
    //layer.load(2); 
    //清除class
    ...
    //不只读时才显示删除
    if (!dataReadonly)
        lab += "    <span  onclick=\"DelT2Page('" + table + "'," + record + ");\" onfocus=\"this.blur();\" class='t2-close'></span>";
    lab += "</li>";
    $("#" + table + "Nav ul").append(lab);
    //加tab内容项
    var content = "";
    content += "<div id='" + table + "Tab-" + record + "' mc='" + table + "Tab_" + record + "Content' class='tab-pane active'>";
    content += "    <div class='panel-body' style='border-width:0px; background:none;'>";
    //字段内容
    ...
    //****************************************
    if (hasDetail)
        content += GetT3Tab(table, detail, detailsy, record, detailsyfield);
    //****************************************
    content += "    </div>";
    content += "</div>";
...
//复制Tab项
function CopyT2Page(table, sy, hasDetail, detail, detailsy, detailsyfield) {
    var titleTip = "";
    ...
    if (copyStatus) {
        ...
        layer.confirm('您确定要复制【' + titleTip + '】吗？', {
            ...
            //新建标签页
            AddT2Page(table, sy, hasDetail, detail, detailsy, detailsyfield);
            //获取当前最大标签数
...
//** 明细 **
//从表Tab项模板
function GetT3Tab(tablab, table, sy, rec, syfield) {
    var html = "";
    //当前Tab项记录数
    //添加锚点
    html += "<a id=\"" + table + "Anchor\" name=\"" + table + "Anchor\"></a>";
    ...
    // html+="<span class='copy' onclick=\"CopyT3Page('" + tablab + "','" + table + "','" + sy + "'," + rec + ",'" + syfield + "');\">复制</span>";

    // html+="<span class='add' onclick=\"AddT3Page('" + tablab + "','" + table + "','" + sy + "'," + rec + ",'" + syfield + "');\">添加</span>";

    html += " <span  class=\"btn btn-default btn-sm btn-add\" id=\"" + table + "_" + rec + "btn\" name=\"" + table + "_" + rec + "btn\"  onclick=\"AddT3Page('" + tablab + "','" + table + "','" + sy + "'," + rec + ",'" + syfield + "');\"></span>";
    html += " <span  class=\"btn btn-default btn-sm btn-copy\" id=\"" + table + "_" + rec + "CopyBtn\" name=\"" + table + "CopyBtn\"    onclick=\"CopyT3Page('" + tablab + "','" + table + "','" + sy + "'," + rec + ",'" + syfield + "');\"></span>";
    ...
    html += "</div>";
    html += "   <div class='left-nav' id=\"" + table + "_" + rec + "Nav\">";
...
//detail:明细表名
//rec:当前从表记录数
function AddT3Page(tablab, table, sy, rec, syfield) {
    //layer.load(2);
    //清除class
    ...
    //不只读时才显示删除
    if (!dataReadonly)
        lab += "<span  onclick=\"DelT3Page('" + table + "'," + rec + ");\" onfocus=\"this.blur();\" class='t2-close'></span>";
...
function CopyT3Page(tablab, table, sy, rec, syfield) {
    var copyStatus = false;
    $("#" + table + "_" + rec + "Nav ul li").each(function (index) {
        if ($(this).hasClass("active")) {
            copyStatus = true;
