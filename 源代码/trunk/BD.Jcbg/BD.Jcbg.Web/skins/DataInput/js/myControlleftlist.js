...
function CreateField(table, hiddenfield, copyhiddenfield, field, presuffix, suffix) {
    ...
    for (var i = 1; i <= fieldLeftList.length; i++) {
        ...
        fieldRowStr += "    </div> </td>";
        fieldRowStr += "    <td class='zhushi_shang ";
        if (fieldLeftList[i - 1].fzyc)
            fieldRowStr += " hiddentd";
        fieldRowStr += "'><div class='zhushi_shang_div' title=''>";
        //描述说明
    ...
    for (var i = 1; i <= fieldRightList.length; i++) {
        ...
        fieldRowStr += "    </div> </td><td class='zhushi_shang ";
        if (fieldRightList[i - 1].fzyc)
            fieldRowStr += " hiddentd";
        fieldRowStr += "'><div class='zhushi_shang_div' title=''>";
...
function GetT2Tab(table, sy, data) {
    ...
    html += "           <button " + xgsqNewHidden + " class=\"btn btn-success btn-outline btn-sm \" type=\"button\" id=\"" + table + "AddBtn\" name=\"" + table + "AddBtn\"  onclick=\"" + table + "New();\"><i class=\"fa fa-plus\"></i>&nbsp;添加</button>";
    ...
    html += "           <button " + xgsqDelHidden + " class=\"btn btn-success btn-outline btn-sm \" type=\"button\" id=\"" + table + "DelBtn\" name=\"" + table + "DelBtn\"   onclick=\"" + table + "Del();\"><i class=\"fa fa-copy\"></i>&nbsp;删除</button>";
    html += "           <button " + xgsqNewHidden + " class=\"btn btn-success btn-outline btn-sm \" type=\"button\" id=\"" + table + "CopyBtn\" name=\"" + table + "CopyBtn\"   onclick=\"" + table + "Copy();\"><i class=\"fa fa-copy\"></i>&nbsp;复制</button>";
    html += "           <button " + xgsqNewHidden + " class=\"btn btn-success btn-outline btn-sm \" type=\"button\" id=\"" + table + "CopyNumsBtn\" name=\"" + table + "CopyNumsBtn\"   onclick=\"" + table + "CopyNum();\"><i class=\"fa fa-copy\"></i>&nbsp;复制多组</button>";

    html += "           <div class=\"btn_right\">";
    //自定义按钮
    $(data.custombutton).each(function (index) {
        html += '           <button class=\"btn btn-success btn-outline btn-sm \" type=\"button\" id=\"' + data.custombutton[index].name + '\" name=\"' + data.custombutton[index].name + '\"   onclick=\"' + ParamRep(data.custombutton[index].event) + '\"><i class=\"fa fa-copy\"></i>&nbsp;' + data.custombutton[index].title + '</button>';
...
function CreateListJs(table, field) {
    ...
    $(field).each(function (index) {
        ...
        if (field[index].kjlx == "SELECT") {
            ...
            fieldStr += "       formatter:function(date) {";
