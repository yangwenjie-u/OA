function add() {
    try {
        var tabledesc = "试验指标";                // 表格描述
        var zdzdtable = encodeURIComponent("ZDZD_JC"); // zdzd名
        var tablename = encodeURIComponent("PR_M_ZB"); 			// 表名
        ...
        var url = "/datainput/Index?zdzdtable=" + zdzdtable +
            ...
            "&button=" + buttons +
            "&rownum=2" +
            "&preproc=InputCheckPrmzb|$pr_m_zb.SYXMBH|$pr_m_zb.zbmc"+
            "&_=" + rdm;
...
function del() {
    try {
        ...
        if (!confirm("删除指标会删除指标和指标所属的产品指标，确定要删除所选的记录吗？")) {
            return;
        }
        $.ajax({
            type: "POST",
            url: "/delete/deleteprmzb?recid=" + encodeURIComponent(selected.RECID),
            dataType: "json",
            async: false,
