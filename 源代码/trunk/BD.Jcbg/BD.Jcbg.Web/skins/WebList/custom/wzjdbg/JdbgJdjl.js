...
function editZgd() {
    try {
        ...
        var lrrzh = selected.LRRZH;
        if (jbr == null || jbr == '') {
            layer.alert('整改单经办人为空!');
            return;
        }
        else if (lrrzh == null || lrrzh == '') {
            layer.alert('当前整改单未签发，无需修改!');
            return;
        }
        else {
            // 校验整改单经办人是否为当前用户
            $.ajax({
                ...
                success: function (data) {
                    if (data.code == "0") {
                        if (data.msg == "0") {
                            layer.alert('您不是当前整改单经办人，不能修改整改单!');
                        }
                        else if (data.msg == "1") {
                            ...
                            if (zgdbh && zgdbh != "") {
                                ...
                                var extrainfo1 = bcode.encode("view_jdbg_jdjl|" + bcode.encode("lx='zgd' and extrainfo4='" + zgdbh + "'") + "||" + "view_i_m_gc|" + bcode.encode("gcbh='" + gcbh + "'"));
                                var extrainfo2 = bcode.encode("修改整改单");
                                var extrainfo3 = bcode.encode(gcbh);
                                var url = "/workflow/startwork?processid=70" +
									// 加密的条件信息（表单中用到）
...
function switchhfRecord(obj) {
    try {
        var strLocation = decodeURIComponent(window.location.href);
        // 所有
        if (obj.checked) {
            strLocation = strLocation.replace("NOT||CHECKBOX", "ALL||CHECKBOX").replace("所有|未回复", "所有|所有");
        }
        else {
            strLocation = strLocation.replace("ALL||CHECKBOX", "NOT||CHECKBOX").replace("所有|所有", "所有|未回复");
        }
        window.location = strLocation;
    } catch (ex) {
        alert(ex);
