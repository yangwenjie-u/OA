function add() {
    try {
        ...
        var tablename = encodeURIComponent("I_S_GC_Dust"); // 表名
        ...
        var url = "/datainput/Index?zdzdtable=" + zdzdtable +
            ...
            "&t1_title=" + title +
            "&button=" + buttons +
            "&preproc=Sp_SaveGcDustInfo|$I_S_GC_Dust.DeviceCode" +
            "&rownum=2" +
            "&LX=N" +
            "&_=" + rdm;
        parent.layer.open({
...
function edit() {
    try {
        ...
        var url = "/datainput/Index?zdzdtable=" + zdzdtable +
            ...
            "&button=" + buttons +
            "&jydbh=" + jydbh +
            "&preproc=Sp_UpdateGcDustInfo|$JYDBH$|$I_S_GC_Dust.DeviceCode" +
            "&rownum=2" +
            "&LX=E" +
            "&_=" + rdm;
        parent.layer.open({
...
function UpdateIOT() {
    try {
        ...
        $.ajax({
            type: "POST",
            url: "/DwgxWzOh/AddOrUpdateDevice?id=" + encodeURIComponent(RECID),
            dataType: "json",
            async: false,
...
function DeleteIOT() {
    try {
        ...
        $.ajax({
            type: "POST",
            url: "/DwgxWzOh/DeleteDevice?id=" + encodeURIComponent(RECID),
            dataType: "json",
            async: false,
            success: function (data) {
                if (data.code == "0") {
