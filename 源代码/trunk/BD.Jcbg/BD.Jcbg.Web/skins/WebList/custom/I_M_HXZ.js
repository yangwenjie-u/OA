function add() {
    try {
        ...
        var tablename = encodeURIComponent("SB_HXZ"); // 表名
        var tablerecid = encodeURIComponent("Id"); // 表主键
        var title = encodeURIComponent("添加黑匣子设备"); // 标题
...
function UpdateIOT() {
    try {
        ...
        var fun = "";
        if (selected.SBLX == "塔吊") {
            fun = "AddTJTowerCraneDevice";
        }
        else {
            fun = "AddSJJElevatorDevice";
        }
        $.ajax({
            type: "POST",
            url: "/DwgxWzOh/" + fun,
            data: { DeviceNo: selected.hxzMachineID, TimeSp: selected.OfflineTime, },
            dataType: "json",
            async: false,
...
function DeleteIOT() {
    try {
        ...
        $.ajax({
            type: "POST",
            url: "/DwgxWzOh/DeleteHxzStatus?id=" + encodeURIComponent(RECID),
            dataType: "json",
            async: false,
            success: function (data) {
                if (data.code == "0") {
...
function zjlist() {
        ...
        try {
            ...
            var url = "/WebList/EasyUiIndex?FormDm=SB_HXZ&FormStatus=2";
            parent.layer.open({
                type: 2,
                title: '查看' + tabledesc,
                shadeClose: true,
