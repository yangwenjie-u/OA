...
/*
		...
		var gcmc=encodeURIComponent(selected.GCMC);
		var jdzch=encodeURIComponent(selected.GCBH);
		var bcode = new Base64();
        var tablename=bcode.encode(table);
		
	    var reportname=encodeURIComponent("人员花名册v1");
		
		var where=encodeURIComponent("jdzch='"+jdzch+"' and gz='"+gz+"'");
		//var where = encodeURIComponent("gcbh='"+gcbh+"'|jdzch='"+gcbh+"' and logyear="+year+" and logmonth="+month+" and (gzname='"+gzname+"' or '"+gzname+"'='') and (BzzUserId='"+bzzuserid+"' or '"+bzzuserid+"'='')  and (GW='"+gw+"' or '"+gw+"'='') ");
