function add() {
    try {
        ...
        // var para = "I_M_GC_YSBA,GCMC," + selected.GCMC + "|I_M_GC_YSBA,GCDD," + selected.GCDD + "|I_M_GC_YSBA,SGDWMC," + selected.SY_SGDWMC +
            // "|I_M_GC_YSBA,SGFRDB," + selected.FRDB + "|I_M_GC_YSBA,FRDH," + selected.FRDBDH +
            // "|I_M_GC_YSBA,SGXMFZR," + selected.SGXMFZR + "|I_M_GC_YSBA,FZRDH," + selected.SGXMFZRDH +
             // "|I_M_GC_YSBA,JDZCH," + selected.JDZCH+"|I_M_GC_YSBA,YSBALX,ZTYS";
		var para = "I_M_GC_YSBA,YSBALX,ZTYS";
...
function apply() {
    try {
        ...
        $.ajax({
            type: "POST",
            url: "/qy/submitysba?recid=" + encodeURIComponent(selected.RECID)+"&ysbalx="+encodeURIComponent("ZTYS"),
            dataType: "json",
            async: false,
            success: function (data) {
                if (data.code == "0") {
                    ...
                    $.ajax({
                       type: "POST",
                       url: "/jdbg/SetInfoMail?mailcon=" + encodeURIComponent(gcmc + "进行工程主体验收备案") + "&maintitle=" + encodeURIComponent("主体验收备案"),
                       dataType: "json",
                       async: false,
                       success: function (data) {
                           // if (data.code != 0)
