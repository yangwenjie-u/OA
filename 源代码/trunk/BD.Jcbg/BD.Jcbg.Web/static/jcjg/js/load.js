...
function statistics0() {
    ...
    var jcjg = $("#jcjg10").val();
    var gcid = $("#gc10").val();
    var params = "sfid=" + province + "&csid=" + city + "&xqid=" + district + "&jdid=" + jd + "&jcjgid=" + jcjg + "&gcid=" + gcid;
    ajaxPost("/jc/GetJgStatistic", params, function(data) {
        var rootData = data.record;
        var gcs = 0;
        if (rootData.gcs != undefined)
            gcs = rootData.gcs;
        $("#enginner_gczs").text(gcs + "个");
        var jzmj = 0;
        if (rootData.jzmj != undefined)
            jzmj = rootData.jzmj;
        $("#enginner_jzmj").text(jzmj + "平米");
        var wtds = 0;
        if (rootData.wtds != undefined)
            wtds = rootData.wtds;
        $("#enginner_wtds").text(wtds + "份");
        var bgs = 0;
        if (rootData.bgs != undefined)
            bgs = rootData.bgs;
        $("#enginner_bgs").text(bgs + "份");
    });
}
...
function statistics1() {
    ...
    var gcid = $("#gc10").val();
    var params = "sfid=" + province + "&csid=" + city + "&xqid=" + district + "&jdid=" + jd + "&jcjgid=" + jcjg + "&gcid=" + gcid;
    ajaxPost("/jc/GetJgBgStatistic", params, function(data) {
        var rootData = data.record;
        var showData = [];
        for (var item in rootData) {
            var jsonObj = {};
            jsonObj["name"] = item;
            jsonObj["value"] = rootData[item];
            showData.push(jsonObj);
        }

        var option = getPie('检测结果统计', showData);
...
function statistics2() {
    ...
    var gcid = $("#gc10").val();
    var params = "sfid=" + province + "&csid=" + city + "&xqid=" + district + "&jdid=" + jd + "&jcjgid=" + jcjg + "&gcid=" + gcid;
    ajaxPost("/jc/GetJgWtdztStatistic", params, function(data) {
        ...
        var option = getBar('委托单状态统计', xAxis, showData);
...
function statistics3() {
    ...
    var gcid = $("#gc10").val();
    var params = "sfid=" + province + "&csid=" + city + "&xqid=" + district + "&jdid=" + jd + "&jcjgid=" + jcjg + "&gcid=" + gcid;
    ajaxPost("/jc/GetJgWtdycztStatistic", params, function(data) {
        var rootData = data.record;
        var showData = [];
        ...
        var option = getBar('委托单异常状态统计', xAxis, ary);
...
function statistics4() {
    ...
    var gcid = $("#gc10").val();
    var params = "sfid=" + province + "&csid=" + city + "&xqid=" + district + "&jdid=" + jd + "&jcjgid=" + jcjg + "&gcid=" + gcid;
    ajaxPost("/jc/GetJgJcjgWtdztStatistic", params, function(data) {
        // var rootData = data.record;
        // var showData = rootData;
        ...
        for (var i in rootData) {
            var val = [];
            for (var item in rootData[i]) {
                val.push(rootData[i][item]);
                if (one) {
                    xAxis.push(item);
                }
            }
