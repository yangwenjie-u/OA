...
(function($, window, document) {
    ...
    $.fn.setTree = function(option) {
        ...
        // var rightData = getSelectData(data);
        // var right = makeRight(rightData);
        // "<div class='right'>" + right.join('') + "</div>"

        $(this).html("<div class='left'>" + left.join('') + "</div>");
        //选中父节点
        checkedData();
    ...
    function makeLeft(data) {
        ...
        for (var i = 0, len = data.length; i < len; i++) {
            ...
            leftStr = '<div class="fir ' + (i == 0 ? 'active' : '') + '" value="' + fir.menucode +
                '"><span class="open">+</span><span class="slide">-</span><input type="checkbox" class="fir-check" /><span>' + fir.menuname +
                '</span><ul>';
            if (fir.child) {
                ...
                for (var j = 0, len2 = fir.child.length; j < len2; j++) {
                    ...
                    if (sed.child) {
                        ...
                        leftStr += "<span class='open'>+</span><span class='slide'>-</span><input type='checkbox' class='sed-check'><span class='user-list' num='" + sed.usernum + "'></span><span>" + sed.menuname + "</span><span class='power-list' num='" + sed.rolenum + "'></span>";
                        child = "<ul class='third'>";
                        for (var k = 0, len3 = sed.child.length; k < len3; k++) {
                            third = sed.child[k];
                            checked = third.hasPower ? 'checked=true' : '';
                            child += "<li value='" + third.menucode + "'><input type='checkbox' class='third-check' " + checked + " /><span class='user-list' num='" + third.usernum + "'></span><span>" + third.menuname + "</span><span class='power-list' num='" + third.rolenum + "'></span></li>";
                        }
                        child += "</ul>";
                        leftStr += child;
                    } else {
                        checked = sed.hasPower ? 'checked=true' : '';
                        leftStr += "<input type='checkbox' class='sed-check' " + checked + " /><span class='user-list' num='" + sed.usernum + "'></span><span>" + sed.menuname + "</span><span class='power-list' num='" + sed.rolenum + "'></span>";
                    }
                    leftStr += '</li>';
                }
            }
