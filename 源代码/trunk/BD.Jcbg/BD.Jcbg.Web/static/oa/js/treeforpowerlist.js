    ...
    function makeLeft(data) {
        ...
        for (var i = 0, len = data.length; i < len; i++) {
            ...
            leftStr = '<div class="fir" value="' + fir.menucode +
                '"><span class="open">+</span><span class="slide">-</span><input type="checkbox" class="fir-check" id="' + fir.menucode + '" /><label >' + fir.menuname +
                (fir.butrows.length?'<span class="button-list" title="按钮权限选择" data-menucode="'+fir.menucode+'" ></span>':'')+'</label> <ul>';
            if (fir.child) {
                ...
                for (var j = 0, len2 = fir.child.length; j < len2; j++) {
                    ...
                    child = '';
                    if (sed.child) {
                        leftStr += "<span class='open'>+</span><span class='slide'>-</span><input type='checkbox' class='sed-check' id='" + sed.menucode + "'><span class='user-list' num='" + sed.usernum + "'></span><label for='" + sed.menucode + "'>" + sed.menuname + "</label><span class='power-list' num='" + sed.rolenum + "'></span>"+(sed.butrows.length?"<span class='button-list'  title='按钮权限选择' data-menucode='"+sed.menucode+"'></span>":"");
                        child = "<ul class='third'>";
                        for (var k = 0, len3 = sed.child.length; k < len3; k++) {
                            third = sed.child[k];
                            checked = third.hasPower ? 'checked=true' : '';
                            child += "<li value='" + third.menucode + "'><input type='checkbox' class='third-check' " + checked + " id='" + third.menucode + "' /><span class='user-list' num='" + third.usernum + "'></span><label for='" + third.menucode + "'>" + third.menuname + "</label><span class='power-list' num='" + third.rolenum + "'></span>"+(third.butrows.length?"<span class='button-list'  title='按钮权限选择' data-menucode='"+third.menucode+"'></span>":"")+"</li>";
                        }
                        child += "</ul>";
                        leftStr += child;
                    } else {
                        checked = sed.hasPower ? 'checked=true' : '';
                        leftStr += "<input type='checkbox' class='sed-check' " + checked + " id='" + sed.menucode + "' /><span class='user-list' num='" + sed.usernum + "'></span><label for='" + sed.menucode + "'>" + sed.menuname + "</label><span class='power-list' num='" + sed.rolenum + "'></span>"+(sed.butrows.length?"<span class='button-list'  title='按钮权限选择' data-menucode='"+sed.menucode+"'></span>":"");
                    }
                    leftStr += '</span></li>';
                }
            }
