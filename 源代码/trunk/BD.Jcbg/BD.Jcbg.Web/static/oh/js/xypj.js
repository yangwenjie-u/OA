    ...
    if (ary.length) {
        for (var i = 0; i < ary.length; i++) {
            ...
            if (obj.recid) {
                ...
                if (params.type == "xypjfh") {
                    str += "<td><span class='edit'>修改</span> <span>|</span> <span class='del'>删除</span></td>";
                    $(".xypjfh").show();
                }
                str += "</tr>";
            }
...
$("#cjgcList").on("click", ".del", function() {
    var index = $(this).parents("tr").index()

    layer.confirm("请确认删除,删除后需手动修改计分分数！", {
        icon: 3,
        title: "删除确认"
    }, function(idx) {
        layer.close(idx);

        if (resData.gc && resData.gc[index] && resData.gc[index].recid) {
            ajaxTpl("/tz/getXYPJCJGCDel", {
                recid: resData.gc[index].recid
            }, function(res) {
                if (res.code == 0) {
                    layer.msg("删除成功!", {
                        icon: 1,
    ...
    layer.open({
        type: 2,
        content: "/tz/xypjgc?id=" + resData.gc[index].recid,
        area: ["90%", "90%"],
        title: "修改承建工程",
        end: function() {
            loadData(getParams().key)
...
$("#tbody").on("click", ".add", function() {
    ...
    isEditFlag = false;
    $(".wdate[name=yxsj]").remove("disabled")
    $("#tipMsg").hide()
    editInit("", tt);
    editIdx = layer.open({
        type: 1,
    ...
    if ($("#tipMsg").length) {
        ...
        $("[name=yxsj]").parents(".line-warp").after('<div id="tipMsg" style="display:inline-block;color: #DD5145;margin-left: 120px;height:36px;line-height: 30px;">请重新选择评价标准后,修改有效截止日期!</div>')
    }
    ...
    isEditFlag = true;
    if ($("#edit [name='df']").attr("readonly") === undefined) { //如果计分可以自己手动输入
        $(".wdate[name=yxsj]").attr("disabled", false)
    } else {
        $(".wdate[name=yxsj]").attr("disabled", true)
    }
...
function editInit(obj, tt) {
    ...
    $("#openPjbz").show();
    var params = getParams();
    if (params.type == "xypjfh") { //复核的时候
        if ($("#edit [name=lx]").val().indexOf("基本分") > -1) {
            $("#edit [name=df]").removeAttr("readonly");
            $("#openPjbz").hide();
        }
    }
...
function kgrqChange(value) {
    ...
    if (yxsjVal > 0 && day) {
        ...
        if ($("#edit [name='df']").attr("readonly") === undefined) { //如果计分可以自己手动输入,不禁止结束日期
            $(".wdate[name=yxsj]").attr("disabled", false)
        } else {
            $(".wdate[name=yxsj]").attr("disabled", true)
        }
