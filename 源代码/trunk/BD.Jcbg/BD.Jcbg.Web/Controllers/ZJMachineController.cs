...
{
    public class ZJMachineController : Controller
    {
        #region 服务
        ...
        public void GetProjectMap()
        {
            ...
            try
            {
                ...
                sql = "select gcmc,gcbh,gczb,gclxbh,gcdd from I_M_GC where gczb is not null and gczb !='' and gcbh in(select jdzch from dbo.SB_ReportSBSY where state!=2)  and zt in( select bh from h_gczt where xssx >=2 and xssx<7 )  " + where + "";
                dt = CommonService.GetDataTable(sql);
                ...
                if (ret)
                {
                    ...
                    if (type == "cxjdz")
                    {
                        if (op == "2")
                        {
                            ...
                            sqls.Add(sql);
                            sqls.Add("update SB_ReportSBSY set UnInstallProgStatus=4 where UnInstallID='" + key + "'");
                        }
                        else
                        {
        ...
        public void PhonegetBAList()
        {
            ...
            try
            {
                ...
                if (code)
                {
                    ...
                    {
                        sql += " and createrid='" + CurrentUser.UserName + "'";
                    }*/
                    if (key != "")
                    {
        ...
        public void PhonegetBZJList()
        {
            ...
            try
            {
                 ...
                 if (code)
                 {
                     ...
                     if (key != "")
                     {
                         sql += " and (PropertyCompanyName like '%" + key + "%' or FactoryName like '%" + key + "%' or FactoryNO like '%" + key + "%' or BZJType like '%" + key + "%')";
                     }
                     datas = CommonService.GetPageData(sql, pagesize, pageindex, out totalcount);
                 }
                ...
                if (code)
                {
                    ...
                    else
                    {
                        ...
                        if (code)
                        {
                            ...
                            if (type == "bzj")
                                sql = "update SB_BZJ set Qrcode='" + value + "' where RECID=" + key;
        ...
        public void PhoneGetSBDetail()
        {
            ...
            try
            {
                ...
                if (code)
                {
                    ...
                    sql = "select formid,workserial,jyrzh from View_ReportQZJ where  sbbaid='" + baid + "' and (gcbh='" + gcbh + "' or gcbh=(select gcbh from I_M_GC where SJGCBH='" + gcbh + "'))";
                    dt = CommonService.GetDataTable(sql);
                    if (dt.Count > 0)
                    {
            ...
            finally
            {
                ...
                jss.MaxJsonLength = 10240000;
                Response.ContentEncoding = System.Text.Encoding.UTF8;
                string ret = string.Format("{{\"code\":\"{0}\",\"msg\":\"{1}\",\"total\":{2},\"sbsy\":{3},\"formid\":\"{4}\",\"workserial\":\"{5}\",\"jyrzh\":\"{6}\",\"Datas\":{7}}}", code ? "0" : "1", msg, hassz.ToString(), jss.Serialize(retdt), formid, workserial, jyrzh, jss.Serialize(dt));
                Response.Write(ret);
                Response.End();
            }
        }
        ...
        public void PhonegetSBSYList()
        {
            ...
            try
            {
                ...
                if (code)
                {
                    ...
                    if (CurrentUser.CurUser.UrlJumpType == "Q" || CurrentUser.CurUser.UrlJumpType == "R")
                    {
                        sql += " and (SGDWBH=(select qybh from i_M_QYZH where yhzh='" + CurrentUser.UserName + "') or JLDWBH=(select qybh from i_M_QYZH where yhzh='" + CurrentUser.UserName + "'))";
                    }
                    if (key != "")
                    {
