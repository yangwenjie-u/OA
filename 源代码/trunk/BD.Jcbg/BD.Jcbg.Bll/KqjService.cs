...
{
    public class KqjService : IKqjCmdService
    {
        ...
        public bool DownRyIris(string rybh, out string msg)
        {
            ...
            try
            {
                string sql = "select qybh,ryxm,sfzhm,hm from i_m_ry where rybh='" + rybh + "'";
                if (!string.IsNullOrEmpty(CurrentUser.Jdzch))
					sql += " and jdzch='"+CurrentUser.Jdzch+"'";
				...
				sql = "insert into KqjDeviceCommand(Serial,Command,UserId,RealName,UserStation,IrisModule) select kqjbh,21,'" + sfzhm + "','" + ryxm + "','','" + hm + "' from i_m_kqj where qybh='" + qybh + "'";
                if (!string.IsNullOrEmpty(CurrentUser.Jdzch))
					sql += " and jdzch='"+CurrentUser.Jdzch+"'";
        ...
        public bool DownKqjIris(string kqjbh, out string msg)
        {
            ...
            try
            {
                IList<IDictionary<string, string>> kqj = CommonDao.GetDataTable("select qybh from i_m_kqj where kqjbh='" + kqjbh + "'");
                if (kqj.Count > 0 && kqj[0]["qybh"].GetSafeString() == "")
                {
                    CommonDao.ExecCommand("insert into KqjDeviceCommand(Serial,Command,UserId,RealName,UserStation,IrisModule) select '" + kqjbh + "',21,usercode,realname,'',irismodule from userinfo where usercode is not null and usercode<>'' and realname<>'' and realname is not null and irismodule is not null and irismodule<>''", CommandType.Text);
                }
                else
                {
                    // 下发人员
        ...
        /// <returns></returns>
        [Transaction(ReadOnly = false)]
        public bool DownWgryKqjIris(string kqjbh, out string msg)
        {
            bool ret = false;
            msg = "";
            try
        ...
        public IList<IDictionary<string, string>> GetBzfzrs()
        {
            ...
            // ----------- end ----------------
            where.Append(" and jdzch='" + CurrentUser.Jdzch + "' ");
            string sql = "select sfzhm,ryxm+'('+gz+')' as ryxm from I_M_WGRY where sfbzfzr='是' " + where + " order by ryxm";
        ...
        {
            string sql = "select * from kqjuserlog where HasDeal='False'";
            return CommonDao.GetDataTable(sql); ;
        }

        public bool updateKqjUserLog(
            string serial, string userid,string companyid, string jdzch,DateTime time,string kqjlx)
        {
            string sql = "Update kqjuserlog Set HasDeal = 'True',companyid='" + companyid + "',placeid='" + jdzch + "',DealType=1,LogType='" + kqjlx+"' ";
        ...
        public IList<IDictionary<string, string>> GetGcQyList(string wdqy)
        {
            string sql = "select qybh,qymc from i_m_qy where wdqy like @wdqy";
            IList<IDataParameter> parameters = new List<IDataParameter>
            {
                new SqlParameter("@wdqy","%"+wdqy+"%")
            };
        ...
        /// 单位人员月考勤信息
        /// </summary>
        public DataTable GetDwKqInfos(string qymc, string rq)
        {
            string sql = "Sp_KqjRqTj";
            IList<IDataParameter> parameters = new List<IDataParameter>
            {
                new SqlParameter("@qymc",qymc),
                new SqlParameter("@rq",rq)
            };
            return CommonDao.GetDataSet(sql, CommandType.StoredProcedure, parameters).Tables[0];
        }
        ...
        /// </summary>
        [Transaction(ReadOnly = false)]
        public DataTable GetDwKqTotal(string qymc, string rq)
        {
            DataTable dt = new DataTable();
            try
            {
                ArrayList list = new ArrayList();
                list.Add("SELECT QYMC,RYXM,GW,rq,yql,cql FROM View_Dwykqbb WHERE QYMC='" + qymc + "' AND rq='" + rq + "'");
                dt = CommonDao.GetDataSet(list).Tables[0];
            }
            catch (Exception e)
            {
                SysLog4.WriteError(e.Message);
		...
		public bool SaveUserLog(string serial, string userid, DateTime time)
        {
            ...
            ryrow = CommonDao.GetRowValue("bzfzr,ryxm,bzfzrxm,sfbzfzr", "View_I_M_RY", "sfzhm='" + userid + "' and jdzch='" + jdzch + "' and qybh='" + qybh + "'");

            if (ryrow == null) //工程中没有该人员
                return false;

            //获取项目是单向还是双向
            IDictionary<string, string> qykqrow = CommonDao.GetRowValue("kqtimes", "i_m_gc_kgba", "jdzch='" + jdzch + "' and qybh='" + qybh + "' ");
            if (qykqrow == null)
                return false;
            ...
            bool ret = updateKqjUserLog(serial, userid, qybh, jdzch, time, kqjlx);
            if(ret)
            {
                if(kqtimes=="2")
                    SaveUserDayLogInAndOut(userid, time, qybh, qymc, jdzch, gcmc, ryxm, bzfzr, bzfzrxm,kqjlx);
                else
        ...
        {
            string qymc, gcmc, bzfzr, ryxm, bzfzrxm = "";
        ...
        protected bool SaveUserDayLogInAndOut(string userid, DateTime time, string qybh, string qymc, string jdzch, string gcmc, string ryxm, string bzfzr, string bzfzrxm,string kqjlx)
        {
            string sql = "";
            try
            {
        ...
        protected bool SaveUserMonthLog(string userid, DateTime time, string qybh, string qymc, string jdzch, string gcmc, string ryxm, string bzfzr, string bzfzrxm,string gzid)
        {
                ...
                {
                    ///插入月考勤相关数据
                    string sql = "INSERT INTO kqjusermonthlog (userid,Logyear,logmonth,QYBH,QYMC,JDZCH,GCMC,RYXM,GzId,Bzfzr,bzfzrxm) values ('" +
                    userid + "','" + 
                    year + "','" +
                    month + "','" +
                    qybh + "','" +
        ...
        public string GetMonthlog_Recid(DateTime time, string userid, string qybh, string jdzch, out int workday)
        {
            ...
            string recid = "";
            List<IDictionary<string, string>> list = new List<IDictionary<string, string>>();
            string sql = "select * from kqjusermonthlog where userid=@userid and qybh=@qybh and jdzch=@jdzch and logyear=@logyear and logmonth=@logmonth";
            List<IDataParameter> parameters = new List<IDataParameter>
                {
        ...
        public bool SetMonthLog(string recid, string day, string logtype, DateTime dt,int workday)
        {
            ...
            try
            {
                ...
                {
                    workday += 1;
                    hql = "update KqjUserMonthLog set " + field + "=convert(datetime,'" + dt.ToString() + "')," + fieldnum + "=1 ,workday="+ workday +" where Recid=" + recid;
        ...
        public IList<IDictionary<string, string>> GetUserMonthPay(string jdzch, string xm, string sfz, string dt1, string dt2, string gz, string gw, string bzfzr, int pageSize, int pageIndex, out int totalCount)
        {
            ...
            string sql = "SELECT   *  FROM ViewKqjUserMonthLog where 1=1 " + where + " order by xssx;";
        ...
        public bool SaveInfoWgryHistory(string sfzhm,string jdzch, bool isOut, DateTime? logdate = null)
        {
          //  string sql="Insert Into I_M_RY_History SET SFZHM"
            ...
            if ((historys == null || historys.Count == 0) && isOut)
                return false;
            // 获取人员信息
            IDictionary<string, string> inforow = CommonDao.GetRowValue("sfzhm,ryxm,qybh,bzfzr,gz", "I_M_RY", "sfzhm='" + sfzhm + "'and jdzch='" + jdzch + "' and qybh!=''");
            if (inforow == null)
                return false;
            string ryxm, qybh, qymc , gcmc, bzfzr, bzfzrxm,gz;

            inforow.TryGetValue("ryxm", out ryxm);
            inforow.TryGetValue("qybh", out qybh);
            inforow.TryGetValue("bzfzr", out bzfzr);
            inforow.TryGetValue("gz", out gz);
            qymc = CommonDao.GetFieldValue("qymc", "i_m_qy", "qybh='" + qybh + "'");
            gcmc = CommonDao.GetFieldValue("gcmc", "i_m_gc_kgba", "jdzch='" + jdzch + "'");
            bzfzrxm = CommonDao.GetFieldValue("ryxm", "I_M_RY", "sfzhm='" + bzfzr + "'and jdzch='"+jdzch+"' and qybh <>''");
            bool ret = true;
            try
            {
                ...
                else
                {
                    ...
                    if (!InCurrGC) //如果没有退场记录的工程为下发模板的工程，则不用执行
                    {
                        ...
                        sql = "Insert INTO i_m_ry_history (SFZHM,RYXM,QYBH,QYMC,JDZCH,GCMC,GZ,intime) values (@sfzhm,@ryxm,@qybh,@qymc,@jdzch,@gcmc,@gz,@intime)";
                        ...
                        Params.Add(new SqlParameter("@gz", gz));
